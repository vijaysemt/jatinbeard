                    ...
                    <div class="col-md-6 col-12 mb-3">
                        ...
                        {{-- <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="save_address"
                                value="{{ old('save_address') }}" id="saveAddress" />
    ...
    document.getElementById('orderForm').onsubmit = function(e) {
        ...
        if (paymentMethod === 'Razorpay') {
            ...
            var options = {
                ...
                .submit(); // Submit the form with the Razorpay payment ID
                },
                "prefill": {
                    "name": document.getElementById('firstName').value + ' ' + document.getElementById(
                                ...
                                .then(verification => {
                                    if (verification.error) {
                                        ...
                                        console.log("razorpay_payment_id:" + response
                                            .razorpay_payment_id);
                                        console.log("razorpay_order_id:" + response
                                            .razorpay_order_id);
                                        // If payment is verified, append payment ID to the form
                                        var paymentIdInput = document.createElement('input');
                                        paymentIdInput.setAttribute('type', 'hidden');
                                        paymentIdInput.setAttribute('name', 'razorpay_payment_id');
                                        paymentIdInput.setAttribute('value', response
                                            .razorpay_payment_id);

                                        var paymentIdInput_order = document.createElement('input');
                                        paymentIdInput_order.setAttribute('type', 'hidden');
                                        paymentIdInput_order.setAttribute('name',
                                            'razorpay_order_id');
                                        paymentIdInput_order.setAttribute('value', response
                                            .razorpay_order_id);
...
<script>
    document.getElementById('pincode').addEventListener('focusout', function() {
        const pincode = this.value.trim();
        // document.getElementById('postalCode').value = document.getElementById('pincode').value;
        if (pincode.length === 6) {
