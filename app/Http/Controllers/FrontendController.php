...
class FrontendController extends Controller
{
    ...
    public function index(){
        //$userId = Auth::check() ? Auth::id() : $this->getGuestUserId();
        $cartItems = Cart::where('user_id', $this->getGuestUserId())->with('product')->get();
        $totalQuantity = $cartItems->sum('quantity');
        $total = $cartItems->sum(function($item) {
            return $item->product->price * $item->quantity;
        });
        return view('index', compact('cartItems', 'total','totalQuantity'));
    }
    ...
    public function product($id){
        ...
        $cartItems = Cart::where('user_id', $this->getGuestUserId())->with('product')->get();
        $totalQuantity = $cartItems->sum('quantity');
        return view('product-details', ['data'=>$product,'totalQuantity'=>$totalQuantity]);
    }
